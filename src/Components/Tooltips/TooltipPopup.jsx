...
export default function TooltipPopup({children}){
    const {open,toggleOpen,title,color="black",style="light"} = useContext(TooltipContext)
    let inboxIcon = blackLight
    let styleClass= (style && color) && `tool-${color}-${style}`

    const allClasses = classnames(styleClass,"w-[380px]  flex rounded-[8px] absolute bottom-[90px] tool px-5 p-3 justify-between items-center gap-2 drop-shadow-xl");
        ...
        <>
        {
            open && <div className={allClasses}>
                <div className="flex justify-around items-start relative gap-3">
                    ...
                    <FontAwesomeIcon icon={faXmark} className="w-5 h-5 hover:cursor-pointer" onClick={toggleOpen}/>
                    <span className={classnames(styleClass,"w-5 h-5 -z-10 absolute -bottom-6 left-5 rotate-45")}></span>
                </div>
            </div>
        }
        </>
